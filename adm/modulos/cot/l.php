...
if ($_GET['b'] != '') {
	...
	for ($i = 0; $i <= count($busqueda_array); $i++) {
		$sql_b .= ' and (id_cotizaciones_generadas like "%' . $busqueda_array[$i] . '%" or nombre like "%' . $busqueda_array[$i] . '%" or telefono like "%' . $busqueda_array[$i] . '%"or email like "%' . $busqueda_array[$i] . '%" or ci like "%' . $busqueda_array[$i] . '%" or fecha like "%' . $busqueda_array[$i] . '%")';
	}
}
...
switch ($orden_campo) {
	case 1:
		...
		break;
	default:
		$sql_o = 'id_cotizaciones_generadas';
		$orden_campo = 0;
}
...
// Consulta
// ***************************************************************************************************
$listado = $db->query('SELECT SQL_CALC_FOUND_ROWS * FROM cotizaciones_generadas WHERE MONTH(fecha) = '.$mes_get.' AND YEAR(fecha) = '.$anio_get. $sql_b . ' order by ' . $sql_o . ' ' . $sql_od . ' limit ' . (($pagina - 1) * $config['pagina_cant']) . ', ' . $config['pagina_cant'] . ';');
...
$cant_autodata = $db->query_first('SELECT count(*) as cant FROM cotizaciones_generadas WHERE cuenta = 1 and MONTH(fecha) = '.$mes_get.' AND YEAR(fecha) = '.$anio_get. $sql_b . ' order by ' . $sql_o . ' ' . $sql_od);
?>
<?php
...
<div id="contenido_cabezal">
	...
	<div style="margin-bottom: 10px;display: inline;">Mes:
		<select name="mes" id="mes" style="width: 110px" onchange="window.location.href='?m=cot_l&mes='+$(this).val()+'&anio='+$('#anio').val()">
	...
	<div style="display: inline;margin-left: 10px;">Año:
		<select id="anio" name="anio" style="width: 110px" onchange="window.location.href='?m=cot_l&mes='+$('#mes').val()+'&anio='+$(this).val()">
	...
	<div style="margin-top: 10px;">
		<strong><?php echo $cant_autodata['cant'] . ' ' ?>Cotizaciones</strong>
	</div>
	<hr class="nb">
</div>
						...
						?>
							<a href="?m=<?php echo $modulo['prefijo']; ?>_l<?php if ($busqueda != '') {
																				echo '&b=' . $busqueda;
																					...
																					echo '&e=' . $inactivo;
																				} ?>&o=1&od=<?php echo $orden_dir == 0 ? 1 : 0; ?>"><strong>Documento <?php echo $od_chr; ?></strong></a>
						<?php
						} else {
						?>
							<a href="?m=<?php echo $modulo['prefijo']; ?>_l<?php if ($busqueda != '') {
			...
			<tbody>
				...
				?>
					<tr>
						...
						?>
						<td>
							<a href="?m=<?php echo $modulo['prefijo']; ?>_v&i=<?php echo $entrada['id_cotizaciones_generadas']; ?>"><?php echo_s($entrada['id_cotizaciones_generadas']); ?></a>
						</td>

						<td>
							<?php echo_s($entrada['ci']); ?>
						</td>
						<td>
